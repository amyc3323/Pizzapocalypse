...
public class PlayerScript : MonoBehaviour
{
    ...
    private void AddEngineForce()
    {
        ...
        if (boostPressed && boostMeter != 0) {
            ...
            mult = 1;
        if (true|| !isDrifting)rb2D.AddForce(speedMultiplier*inputVector.y*(1+velocityToExtraForce.Evaluate(rb2D.velocity.magnitude))*((inputVector.y<0)?0f:1)*transform.up*accelerationFactor * mult * (isDrifting ? 0.1f : 1f));
